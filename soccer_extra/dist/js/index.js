...
/***/ function(module, exports, __webpack_require__) {
	...
	var PlayerBase = function (_MovingEntity) {
	  ...
	  _createClass(PlayerBase, [{
	    ...
	    key: 'closestOpponentPlayer',
	    value: function closestOpponentPlayer() {
	      var oppTeamMemberList = this.team().opponents().members();
	      var closestSoFar = _utils.MaxFloat;
	      var closestOpp = null;
	      for (var i = 0; i < oppTeamMemberList.length; i++) {
	        var it = oppTeamMemberList[i];
	        var distSqToOpp = (0, _vector2d.vec2DDistanceSq)(this.pos(), it.pos());
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ChaseOpp = {
	  ...
	  execute: function execute(player) {
	    var closestOpp = player.closestOpponentPlayer();
	    if (closestOpp) {
	      var closestOppToHomeGoal = player.team().homeGoal().center().add(closestOpp.pos().getReverse());
	      var distClosestOppToHomeGoal = closestOppToHomeGoal.length();
	      var targetPos = closestOpp.pos().add((0, _vector2d.vec2dNormalize)(closestOppToHomeGoal).crossNum(player.bRadius() * 1 + 5 * distClosestOppToHomeGoal / (player.team().pitch().playingArea().width() / 2)));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var FieldPlayer = function (_PlayerBase) {
		...
		_createClass(FieldPlayer, [{
			...
			key: 'isToFarFromHomeRegion',
			value: function isToFarFromHomeRegion() {
				var distToHomeRegionSq = (0, _vector2d.vec2DDistanceSq)(this.pos(), this.homeRegion().center());
				var maxDist = this.team().pitch().playingArea().width() / 2;
